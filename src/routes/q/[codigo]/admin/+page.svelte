<script lang="ts">
	import { calcularBote, calcularRecaudado, formatoMoneda } from '$lib/quiniela';

	let { data } = $props();

	// Copias editables locales (mock; en backend serían acciones POST protegidas por admin_token).
...
<div class="space-y-6">
	<div class="rounded-xl bg-oro-400/10 p-4 text-sm text-[var(--texto-suave)]">
		...
		(protegido por el token de admin). Los cambios de dinero son informativos.
	</div>
